import { supabase } from "@/integrations/supabase/client";

let lastHash = "";

export async function logAuditEvent(
  eventType: string,
  useCaseId: string,
  callId?: string,
  payload: Record<string, unknown> = {}
) {
  const timestamp = new Date().toISOString();
  const eventPayload = { eventType, useCaseId, callId, payload, timestamp };
  const payloadStr = JSON.stringify(eventPayload);

  // Simple hash chaining simulation using btoa (demo-grade, not crypto-grade)
  const hashInput = lastHash + payloadStr + timestamp;
  const hashCurr = btoa(hashInput).slice(0, 64);
  const hashPrev = lastHash;
  lastHash = hashCurr;

  try {
    await supabase.from("audit_events").insert({
      event_type: eventType,
      use_case_id: useCaseId,
      call_id: callId || null,
      payload: payload as any,
      hash_prev: hashPrev,
      hash_curr: hashCurr,
      actor_user_id: "demo-agent",
    });
  } catch (err) {
    console.warn("Audit log error:", err);
  }
}

export const AUDIT_EVENTS = {
  USE_CASE_SELECTED: "USE_CASE_SELECTED",
  USE_CASE_CHANGED: "USE_CASE_CHANGED",
  PRODUCT_LINE_SELECTED: "PRODUCT_LINE_SELECTED",
  PRODUCT_LINE_CHANGED: "PRODUCT_LINE_CHANGED",
  ORG_TYPE_SELECTED: "ORG_TYPE_SELECTED",
  ORG_TYPE_CHANGED: "ORG_TYPE_CHANGED",
  AUTHORITY_MODEL_SET: "AUTHORITY_MODEL_SET",
  RECOMMENDATION_RENDERED: "RECOMMENDATION_RENDERED",
  RECOMMENDATION_APPLIED: "RECOMMENDATION_APPLIED",
  SCRIPT_COPIED: "SCRIPT_COPIED",
  ESCALATION_TRIGGERED: "ESCALATION_TRIGGERED",
  SCENARIO_CREATED: "SCENARIO_CREATED",
  SCRIPT_VERSION_CREATED: "SCRIPT_VERSION_CREATED",
  SIM_STARTED: "SIM_STARTED",
  SIM_EVENT_INJECTED: "SIM_EVENT_INJECTED",
  DISPOSITION_SET: "DISPOSITION_SET",
  // End-to-end script engine events
  PHI_VERIFICATION_STARTED: "PHI_VERIFICATION_STARTED",
  PHI_VERIFICATION_PASSED: "PHI_VERIFICATION_PASSED",
  PHI_VERIFICATION_FAILED: "PHI_VERIFICATION_FAILED",
  INTAKE_COMPLETED: "INTAKE_COMPLETED",
  LOOKUP_RUN: "LOOKUP_RUN",
  DOCS_REQUESTED: "DOCS_REQUESTED",
  WARM_TRANSFER_INITIATED: "WARM_TRANSFER_INITIATED",
  WARM_TRANSFER_COMPLETED: "WARM_TRANSFER_COMPLETED",
  CALL_RESOLVED: "CALL_RESOLVED",
  // PBM-specific events
  PBM_COVERAGE_CHECK_RUN: "PBM_COVERAGE_CHECK_RUN",
  PBM_REJECT_TROUBLESHOOT_RUN: "PBM_REJECT_TROUBLESHOOT_RUN",
  PBM_PA_CASE_CREATED: "PBM_PA_CASE_CREATED",
  PBM_APPEAL_PACKET_GENERATED: "PBM_APPEAL_PACKET_GENERATED",
  PBM_SPECIALTY_ONBOARDING: "PBM_SPECIALTY_ONBOARDING",
  PBM_DUR_ESCALATION: "PBM_DUR_ESCALATION",
} as const;
