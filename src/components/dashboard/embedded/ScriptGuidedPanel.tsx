...
import {
  CheckCircle2, Circle, ChevronDown, ChevronUp, Copy, ArrowRight,
...
export function ScriptGuidedPanel({ scenario, autoPlay = false }: ScriptGuidedPanelProps) {
  ...
  const currentBlock = scenario.blocks[currentBlockIdx];
  const isComplete = completedBlocks.size === scenario.blocks.length;
  ...
  const handleCompleteBlock = useCallback((blockId: string) => {
    ...
    // PHI verification auto-pass for demo
    if (blockId === "B") {
    ...
    if (blockId === "D") {
      logAuditEvent("INTAKE_COMPLETED", scenario.useCaseId);
    }

    if (blockId === "E" || blockId === "E-deesc") {
      logAuditEvent("LOOKUP_RUN", scenario.useCaseId);
    }

    // Advance to next block
    if (currentBlockIdx < scenario.blocks.length - 1) {
      setCurrentBlockIdx(prev => prev + 1);
    }
  }, [currentBlockIdx, scenario]);
    ...
    switch (actionType) {
      ...
      case "generate_packet": {
        const packet = generateTransferPacket(
          phiVerified,
          phiPoints,
          scenario.transferPacketTemplate?.summary || "Call context transfer",
        ...
        logAuditEvent("WARM_TRANSFER_INITIATED", scenario.useCaseId, undefined, { target: scenario.config.warm_transfer_target });
      ...
      case "copy":
        ...
        break;
      case "warm_transfer":
        logAuditEvent("WARM_TRANSFER_COMPLETED", scenario.useCaseId, undefined, { target: scenario.config.warm_transfer_target });
        toast({ title: "Warm Transfer Initiated", description: `Connecting to ${scenario.config.warm_transfer_target}…` });
        break;
      case "request_docs":
  ...
  }, [scenario, phiVerified, phiPoints, capturedData, completedBlocks, transferPacket]);

  const handleSetDisposition = (d: string) => {
    setDisposition(d);
    logAuditEvent("DISPOSITION_SET", scenario.useCaseId, undefined, { disposition: d, refNumber });
  ...
  const blockIcon = (block: ScriptBlock) => {
    ...
    const active = scenario.blocks[currentBlockIdx]?.blockId === block.blockId;
    ...
    <div className="space-y-2">
      ...
      <div className="h-1 rounded-full bg-secondary overflow-hidden">
        <div
          ...
          style={{ width: `${(completedBlocks.size / scenario.blocks.length) * 100}%` }}
        />
      </div>
                  ...
                  {block.blockId === "H" && transferPacket && (
                    <div className="p-2 rounded bg-secondary/40 border border-border space-y-1">
                      ...
                      <div className="p-1.5 rounded bg-primary/5 border border-primary/10 mt-1">
                        ...
                        <p className="text-[9px] text-foreground mt-0.5">"I'm connecting you now with our {scenario.config.warm_transfer_target} team. I've shared all the details we discussed so you won't need to repeat anything. They'll be able to assist you from here. Is there anything else before I transfer?"</p>
                      </div>
                    </div>
                  )}
      ...
      {isComplete && (
        <div className="five9-card p-3 border-emerald-500/30 bg-emerald-500/5 space-y-2 animate-in fade-in">
          ...
          <div className="grid grid-cols-2 gap-2 text-[10px]">
            <div><span className="text-muted-foreground">Disposition:</span> <span className="font-semibold text-foreground">{disposition || "Resolved"}</span></div>
            ...
            <div><span className="text-muted-foreground">Blocks:</span> <span className="text-foreground">{completedBlocks.size}/{scenario.blocks.length}</span></div>
          </div>
        </div>
      )}
